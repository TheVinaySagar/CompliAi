...
const ChatPage = memo(() => {
  ...
  const { messages, isLoading, error, sendMessage, clearMessages, setError } =
    useChat();
  const { user } = useAuth();
  ...
  const loadDocuments = useCallback(async () => {
    try {
      ...
      if (response.success && response.data) {
        const formattedDocuments: UploadedDocument[] = response.data.map(
          (doc: any) => ({
    ...
    async (e: React.FormEvent) => {
      ...
      try {
        await sendMessage(message, {
          ...
          document_id:
            selectedDocument !== "general" ? selectedDocument : undefined,
  ...
  const handleGeneratePolicy = useCallback(
    (action: string) => {
      const policyPrompt = `Help me generate a ${action.replace(
        "Generate ",
        ""
      )} policy document. Please provide a comprehensive template that includes all necessary sections and compliance requirements.`;
  ...
  return (
    <div className="flex flex-col h-full bg-background">
      {/* Enhanced Header */}
      <div className="bg-background shadow-sm border-b border-border px-4 sm:px-6 py-2">
                  ...
                  <div className="flex items-center gap-4">
                    ...
                    <div className="flex items-center gap-3">
                      ...
                      <p className="text-xs text-blue-600 dark:text-blue-300">
                        {doc.chunks_created} chunks • {doc.controls_identified}{" "}
                        controls identified
                      </p>
                    </div>
                  </div>
        ...
        <div className="max-w-4xl mx-auto">
          {messages.length === 0 && (
            <div className="text-center py-8 sm:py-12">
              <Card className="mx-auto max-w-2xl border shadow-sm bg-background">
                ...
                <CardContent className="space-y-6">
                  <div className="text-center">
                    <p className="text-sm font-medium text-foreground mb-4">
                      {selectedDocument !== "general"
                        ? "Try asking about your document:"
                    ...
                    <div className="mt-6 p-6 bg-gradient-to-r from-muted/50 to-muted rounded-lg border-2 border-dashed border-border">
                      <div className="text-center">
                        ...
                        <p className="text-xs text-muted-foreground mb-4">
                          Upload compliance documents to chat with them
                        </p>
                        <Button
                          variant="outline"
                          size="sm"
          ...
          {isLoading && messages.length > 0 && <ChatMessageSkeleton />}
        </div>
      </div>

      {/* Enhanced Input Area */}
      <div className="bg-background border-t border-border p-4 sm:p-2">
