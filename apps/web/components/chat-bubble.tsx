...
const SourceCard = memo(({ source, index }: { source: Source; index: number }) => {
  ...
  return (
    <div 
      className={cn(
        "text-xs bg-primary/5 text-primary px-3 py-2 rounded-lg border border-primary/20 hover:bg-primary/10 transition-all duration-200 hover:shadow-sm",
        getSourceUrl(source) && "cursor-pointer hover:border-primary/30"
      )}
    ...
    >
      <div className="flex items-start space-x-2">
        ...
        <div className="flex-1 min-w-0">
          ...
          <div className="flex flex-wrap gap-2 mt-1 text-primary/80">
            {source.page && (
            ...
            {source.framework && (
              <span className="text-xs bg-primary/10 px-1 rounded">{source.framework}</span>
            )}
            {source.type && (
              <span className="text-xs bg-primary/10 px-1 rounded">{source.type}</span>
            )}
          </div>

          {/* Show description or excerpt if available */}
          {source.description && (
            <div className="text-primary/70 text-xs mt-1 line-clamp-2">
              {source.description}
            </div>
          )}
          ...
          {source.excerpt && (
            <div className="text-primary/70 text-xs mt-1 italic line-clamp-2">
              "{source.excerpt}"
            </div>
          )}
        </div>
      ...
      <div
        className={`max-w-4xl rounded-2xl px-6 py-5 ${
          isUser 
            ...
            : "bg-card/80 backdrop-blur-sm border border-border text-card-foreground shadow-md hover:shadow-lg transition-all duration-200"
        }`}
      >
        ...
        {!isUser && message.confidence_score && (
          <div className="mt-4 pt-3 border-t border-border">
            <div className="flex items-center text-xs text-muted-foreground">
              <div className="flex items-center bg-emerald-500/10 text-emerald-600 dark:text-emerald-400 px-3 py-1 rounded-full border border-emerald-500/20">
        ...
        <div className={`text-xs mt-4 ${isUser ? "text-blue-100" : "text-muted-foreground"} flex items-center justify-between`}>
